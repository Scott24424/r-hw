...
// 409 — 실제 실패가 1건이라도 있으면 아무것도 바뀌지 않는다 (UR-25.4)
{
  "error": "BULK_STATUS_REJECTED",
  "applied": false,                       // 항상 false. 부분 적용은 존재하지 않는다
  "failures": [
    { "id": "...", "reason": "INVALID_TRANSITION", "from": "SKIPPED", "to": "DONE" },
    { "id": "...", "reason": "NOT_FOUND" }
  ]
}
```
...
| `POST /assignments/bulk-status` | **부록 C.3의 최소 테스트 조건 5건** (D22) — 전부 유효 시 전부 변경 / 실제 실패 1건이면 전부 롤백(UR-25.4) / 동일 상태만이면 성공·변경 0건 / 혼합이면 전체 성공(UR-25.5) / 반복 실행 시 최종 상태 동일. **담당은 T09이며 배치 1 범위 밖이다** |
...
| API — 과제 · 상태 전이 · reorder · bulk · bulk-status | §5.2, 부록 C.3, **D22**, D20. **`bulk-status`의 최소 테스트 조건 5건이 이 태스크의 필수 완료 조건이다** | T09 |
...
| **실제 실패의 정의** | 존재하지 않는 과제 · 허용되지 않는 상태 전이(§3.2) · 권한 오류 · 검증 오류 | **UR-25.4** |
...
**최소 테스트 조건 (구현 태스크의 필수 완료 조건, D22).** 아래 5건은 `bulk-status`를 구현하는 태스크가 **반드시** 검증한다. 담당은 **엔드포인트 T09 · 진입 화면 T17**(둘 다 예약 번호)이며 **배치 1(T01~T04)의 범위 밖**이다.

| # | 조건 | 기대 |
|---|---|---|
| 1 | 모든 항목이 유효하다 | **전부 변경**되고 커밋된다. `changed`가 대상 수와 같다 |
| 2 | 항목 하나가 **실제로 실패**한다 (없는 과제 / 금지된 전이) | **전부 롤백.** 다른 항목의 상태가 하나도 바뀌지 않고 `applied: false`, 실패 사유가 항목 단위로 반환된다 (UR-25.4) |
...
2번은 **롤백을 반증 가능하게** 만들어야 한다 — 성공 항목이 먼저 처리된 뒤에 실패 항목이 오도록 구성해, 트랜잭션 밖에서 처리하는 구현이 통과하지 못하게 한다 (T04 요구사항 2-A와 같은 원칙).
