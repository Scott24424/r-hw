...
> **무엇이 요구되었는가**는 [`requirements.md`](./requirements.md)에 있다 — 이 문서는 그 문서에서 파생된다.
> 판단이 필요한 지점에서 이 문서들 어디에도 답이 없다면, 그것은 설계 누락이다 — 임의로 진행하지 말고 질문할 것.

- 상태: **승인됨** (2026-08-01). 설계 가정 12건(AP-01~AP-12)과 미결 세부 `OPEN-01`·`OPEN-02`에 대한 사용자 결정이 전부 반영되었다 — [`requirements.md` §5](./requirements.md#5-ap-01--ap-12-처리-결과)·[§7](./requirements.md#7-open--미결-세부). **미승인 설계 가정 0건, 미결 구현 세부 0건**
- 스택(고정): Next.js App Router + TypeScript + Prisma + SQLite / vitest(단위) + Playwright(E2E). **UR-14로 확정됨.** 초기 목표는 Mac mini에서 실행하고 브라우저로 쓰는 단일 가족용 시스템이다
- 근거 자료: `docs/mockups/plan-20days.jpg`, `docs/mockups/daily-schedule.jpg`
- 함께 볼 것:
  - [`requirements.md`](./requirements.md) — **요구사항의 유일한 권위 있는 출처.** §0의 F1~F18은 그 문서 §3의 `MR-##` 관찰과 대응한다
  - [`decisions.md`](./decisions.md) — 각 설계 결정의 근거와 기각한 대안 (결정 목록은 §2의 표가 유일한 색인이다)

> **요구사항 추적 (역승격 금지).** 이 문서의 내용을 요구사항으로 되돌려 적지 않는다. 각 설계가 어느 요구에서 왔는지는 [`requirements.md` §8.2](./requirements.md#82-역방향--설계스펙--요구사항)의 역방향 표가 유일한 기록이다.
>
> **2026-08-01 결정으로 바뀐 것 (UR-16, AP-03 거절):** 시간표는 **날짜 없이 매일 반복되지 않는다.** `ScheduleBlock`이 `date`를 갖고, 겹침 불변식 I9는 같은 날짜 안으로 좁혀지며, 시간표 API는 `date`를 필수 필터로 받는다. 변경 범위 전체는 [`requirements.md` §6](./requirements.md#6-ur-16이-바꾼-데이터-모델-ap-03-거절의-결과)에 모여 있다.
...
### 0.2 `daily-schedule.jpg` — 하루 시간표

> **관찰 사실: 이 자료에는 날짜가 적혀 있지 않다** (MR-20). 날짜가 없다는 것은 "매일 반복된다"는 뜻이 **아니다.** 사용자 요구는 "**특정 하루**의 숙제와 활동을 시간대별로 나눈 계획표"이며(UR-07), 2026-08-01 결정으로 시간표 항목은 **특정 날짜에 연결된다** (UR-16).
...
| # | 관찰 | 설계 함의 |
|---|---|---|
| F11 | `7:30 기상`, `8:00 아침식사 끝내기`는 **범위가 아니라 시점**이다. 나머지 **8건**은 범위다 — **시점 2 + 범위 8 = 10건**이고 이 수가 UR-24.5·부록 C.1·T04의 블록 수와 같다 (MR-21·MR-22) | `endMinute`을 nullable로 두어 시점 마커를 표현 (**D10**) |
...
이 대응 관계를 **저장할 것인가 파생할 것인가**가 D11의 쟁점이다. **양쪽 모두 날짜를 가지므로**(UR-16) 매칭은 같은 날짜 안에서만 일어난다.
...
erDiagram
    ...
    ScheduleBlock {
        string id PK
        string date "YYYY-MM-DD — 이 시간표가 속한 날짜 (UR-16)"
...
model ScheduleBlock {
  id          String          @id @default(cuid())
  date        String          // "YYYY-MM-DD" — UR-16. Assignment.date와 같은 표기 (D9)
  startMinute Int
  endMinute   Int?
  label       String
  ...
  @@index([date, startMinute])   // "이 날짜의 시간표를 시간 순서로" (UR-16.2, UR-08)
}
```

> **`ScheduleBlock.date`는 UR-16의 결과다.** 초안은 시간표를 날짜 없이 매일 반복되는 것으로 모델링했으나, 사용자 요구는 "**특정 하루**의 숙제와 활동을 시간대별로 나눈 계획표"(UR-07)이고 2026-08-01 결정에서 그 모델이 **명시적으로 거절**되었다. 시간표 항목은 특정 날짜에 속한다.
>
> **같은 시간표를 여러 날짜에 재사용하는 템플릿은 현재 요구가 아니다** (UR-16.3, NR-03). 날짜별로 블록을 만든다. 템플릿은 후속 기능이며 `ScheduleBlock`에 `templateId`를 추가하는 것이 확장 경로다 — 기존 데이터 마이그레이션이 필요 없다.
...
| 필드 | 의미 | 비고 |
|---|---|---|
| `date` | 이 시간표가 속한 날짜. `"YYYY-MM-DD"` 문자열 | **UR-16.** `Assignment.date`와 같은 표기·정렬 성질을 갖는다 (D9). 날짜별로 블록을 만들며, 템플릿·반복은 후속이다 (UR-16.3) |
...
**먼저 enum의 보장 수준을 정확히 적는다 (DR-07).** 이 표는 [`decisions.md` D13](./decisions.md#d13)의 같은 표, `docs/specs/T03-prisma-schema.md` 요구사항 3과 **같은 내용이어야 한다.** 셋 중 하나만 바뀌면 데이터 무결성의 단일 출처가 갈라진다.
...
| 6 | 외부 입력의 값 범위 검증은 **Zod가 담당한다** (`z.nativeEnum`, §1.4). | **T07** |
| 7 | 서비스 계층 우회 금지(D14)는 값 범위가 아니라 **여러 필드에 걸친 불변식과 쓰기 경로**를 보호한다. | D14 |

공식 근거: [Prisma — SQLite connector](https://www.prisma.io/docs/orm/overview/databases/sqlite) (확인일 2026-08-01).

즉 **"DB가 강제하니까 안심"이 성립하는 컬럼은 하나도 없다.** 단일 컬럼의 값 범위는 Prisma를 거치는 쓰기에서만 좁혀지고, 아래 항목들은 **여러 컬럼에 걸친 조건**이라 Prisma 스키마로는 애초에 표현할 수 없다. 둘 다 **애플리케이션 계층의 Zod discriminated union + vitest**로 강제한다. 각 항목마다 통과 케이스와 위반 케이스를 모두 작성한다.
...
| I9 | **같은 `date` 안에서** 범위 블록끼리 시간 겹침 금지. 마커는 검증 대상 제외. **다른 날짜의 블록은 겹침 판정 대상이 아니다** (UR-16) | 409 |
| I13 | `ScheduleBlock.date`는 유효한 `YYYY-MM-DD` (I6과 같은 규칙, UR-16) | 400 |
...
| # | 결정 | 이 문서에서 영향받는 곳 |
|---|---|---|
| [D1](./decisions.md#d1) | 날짜별 과제와 날짜별 시간표를 별도 엔티티로 분리 | §1.1 |
...
| [D11](./decisions.md#d11) | 블록↔과제는 같은 날짜 안에서 `matchType` 파생 매칭 | §1.1, §5.4, §6.2-2 |
...
| [D15](./decisions.md#d15) | 같은 날짜 안에서 범위 블록 겹침 금지, 시점 마커는 예외 | §1.5(I9), §6.2-2 |
...
| [D18](./decisions.md#d18) | 방학 기간의 파일럿 기본값을 코드 상수로 (2026-07-29 ~ 08-17) | §6.2-3 |
| [D19](./decisions.md#d19) | DB 연결 문자열은 `.env` 없이 코드·스크립트 기본값으로 | §1.2 |
| [D20](./decisions.md#d20) | E2E 시각 주입은 플래그로만 열리는 테스트 전용 헤더 | §7.3 |
| [D21](./decisions.md#d21) | 허용 Node를 {22, 24, 26}으로 고정, CI 기준선 `.nvmrc = 22`가 정의처 | §7.1, §7.3 |
| [D22](./decisions.md#d22) | `bulk-status`는 원자적(실패 1건이면 전부 롤백)이고 동일 상태 재적용은 멱등 no-op | §5.2, §7.2, 부록 C.3 |

**결정의 개수를 본문에 적지 않는다.** 이 표가 유일한 색인이며, 개수를 여러 곳에 적으면 결정이 추가될 때마다 갈라진다 (DR-14).
...
| `POST` | `/api/assignments/bulk-status` | **한 날짜의 과제 상태를 한 번에 전이** (UR-25). 요청은 `{ "date": "YYYY-MM-DD", "to": "DONE" }`이고 대상은 **그 날짜의 과제 전체**다. **원자적이며**(실패 1건이면 전부 롤백, UR-25.4) **동일 상태 재적용은 성공하는 no-op이다**(UR-25.5). 상태를 실제로 바꾸는 항목은 §3.2의 단일 전이 검증을 그대로 통과해야 한다 (UR-25.2) — **D22** |
...
**`/bulk`와 `/bulk-status`는 다른 것이다.** 앞은 **여러 날짜에 과제를 만드는** 것이고, 뒤는 **한 날짜의 과제 상태를 한 번에 바꾸는** 것이다 (UR-25). 담당도 다르다 — **엔드포인트는 T09**(부록 A.3의 "API — 과제 · 상태 전이 · reorder · bulk · bulk-status"), **그 진입 화면은 T17**(부록 A.2)이다.

**`bulk-status`의 요청·응답 최소 계약 (D22, UR-25.4·UR-25.5).**

**요청은 `{ date, to }` 하나뿐이며 Assignment ID 목록을 받지 않는다.** 대상은 **트랜잭션 시작 시점에 그 날짜에 존재하는 과제 전체**다. 이 요청 모델이 아래 실패 분류를 결정한다 (R3-01).

```jsonc
// POST /api/assignments/bulk-status
{ "date": "2026-07-29", "to": "DONE" }   // 필수 필드 2개. 그 밖의 필드는 받지 않는다

// 200 — 전체 성공. changed + unchanged = 대상 전체이며 그 합이 곧 커밋된 요청이다
{
  "date": "2026-07-29",
  "to": "DONE",
  "changed":   2,   // 실제로 상태가 바뀐 항목 수
  "unchanged": 1,   // 이미 DONE이어서 no-op으로 성공 처리된 항목 수 (UR-25.5)
  "items": [ /* Assignment DTO[] — 요청 후 최종 상태 */ ]
}

// 200 — 빈 날짜. 유효한 날짜지만 대상 과제가 0건이다. 오류가 아니다
{ "date": "2026-08-03", "to": "DONE", "changed": 0, "unchanged": 0, "items": [] }

// 400 — 요청 수준 검증 실패. 트랜잭션을 열지 않으므로 write가 0건이다
{
  "error": "BULK_STATUS_INVALID_REQUEST",
  "applied": false,
  "issues": [ { "field": "date", "code": "INVALID_DATE_FORMAT" } ]
}

// 409 — 대상 중 허용되지 않는 상태 전이가 1건 이상이다. 전체 롤백 (UR-25.4)
{
  "error": "BULK_STATUS_REJECTED",
  "applied": false,                       // 항상 false. 부분 적용은 존재하지 않는다
  "failures": [
    { "id": "<실재하는 대상 Assignment의 id>", "reason": "INVALID_TRANSITION", "from": "SKIPPED", "to": "DONE" }
  ]
}

// 500 — 트랜잭션 내부의 예상하지 못한 실패. 전체 롤백
{ "error": "BULK_STATUS_INTERNAL_ERROR", "applied": false }
```

**실패 분류 (R3-01).** 요청에 ID가 없으므로 실패는 **요청 수준**과 **대상 항목 수준**으로 나뉜다.

| 분류 | 무엇이 실패인가 | HTTP | 본문 | 트랜잭션 write |
|---|---|---:|---|---|
| **요청 검증** | 잘못된 날짜 형식, 허용되지 않는 `to`, 필수 필드 누락, 알 수 없는 필드 | **400** | `BULK_STATUS_INVALID_REQUEST` · `applied: false` · `issues[]`(필드·코드) | **0건.** 트랜잭션을 열기 전에 거절한다 |
| **빈 날짜** | 유효한 요청이고 대상이 0건 | **200** | `changed: 0` · `unchanged: 0` · `items: []` | 0건. **성공하는 빈 batch이며 롤백도 오류도 아니다** |
| **상태 전이** | 대상 중 1건 이상이 §3.2에서 허용되지 않는 전이 | **409** | `BULK_STATUS_REJECTED` · `applied: false` · `failures[]` | 전부 롤백. **어떤 Assignment도 변경되지 않는다** |
| **내부 실패** | DB write 실패 등 트랜잭션 내부의 예상하지 못한 오류 | **500** | `BULK_STATUS_INTERNAL_ERROR` · `applied: false` | 전부 롤백 |

- **`failures[]`의 `id`는 실재하는 대상 Assignment의 id뿐이다.** 특정 항목에 귀속할 수 없는 실패에는 **가짜 항목 ID를 만들지 않는다** — 500이 그 경우이며 `failures`를 담지 않는다.
- **`NOT_FOUND`는 항목별 실패 유형에서 제외한다 (R3-01).** 대상 집합을 서버가 `date`로 조회해 만들므로 그 안의 특정 ID가 "없음"일 수 없다. 날짜 자체에 행이 0건인 경우는 위의 **빈 날짜 200**이다.
- **권한 오류도 현재 항목별 실패 유형에서 제외한다.** MVP에는 인증·권한 분리가 없다 (UR-19.1, NR-04). 향후 인증이 추가되면 **요청 수준 401/403**으로 처리하고 그때도 write는 0건이다 — 후속 고려사항일 뿐 현재 계약에는 없다.
- `UR-25.4`의 "실제 실패" 정의는 그대로다. 현재 `{ date, to }` 계약에서 그 네 유형이 어디로 가는지만 고정한 것이다 — 검증 오류 → 요청 수준 400, 허용되지 않는 전이 → 항목 수준 409, 존재하지 않는 과제·권한 오류 → **현재 계약에서 발생 불가**.
- **`unchanged`를 `failures`에 넣지 않는다.** 동일 상태는 성공이다 (UR-25.5).
- **`applied: true`인 부분 성공 응답은 존재하지 않는다.** 전부 커밋되었거나 아무것도 바뀌지 않았거나 둘 중 하나다 (UR-25.4).
- 실패 응답은 **어느 항목이 왜 실패했는지**를 담는다 — 사용자가 고쳐 다시 보낼 수 있어야 원자성의 대가가 감당된다 (D22).
- **내부 예외의 세부정보(스택, 드라이버 메시지, SQL)를 클라이언트에 노출하지 않는다.** 500 본문은 안정적인 오류 코드 하나로 고정한다.
- 쓰기는 **단일 트랜잭션**이다. `reorder`(§7.2)와 시드(T04)가 쓰는 규칙과 같다.
...
**한글책 관리 (UR-23):** 위 엔드포인트가 한글책·영어책을 구분 없이 처리한다. `language=KO` 필터로 한글책만 볼 수 있고, 관리 화면(§6.2-5)에서 추가·수정·보관·삭제가 모두 가능하다. **시드에는 제목이 불명확한 책을 넣지 않는다** (§ 부록 C).
...
| 메서드 | 경로 | 설명 |
|---|---|---|
| `GET` | `/api/schedule/blocks?date=` | **그 날짜의** 블록 (`startMinute ASC`). **`date`는 필수다** (UR-16.2) |
| `POST` | `/api/schedule/blocks` | 생성. `date` 필수. **같은 날짜 안에서만** 겹침 검증 (I9, D15) |
| `PATCH` | `/api/schedule/blocks/:id` | 수정. 자기 자신 제외하고 **같은 날짜 안에서** 겹침 검증 |
| `DELETE` | `/api/schedule/blocks/:id` | 삭제 |
| `GET` | `/api/schedule/today?date=` | 그 날짜의 블록 + **각 블록에 매칭된 같은 날짜의 과제** (D11 규칙). `date` 생략 시 서버의 오늘 |

**`date`를 필수 필터로 두는 이유 (UR-16).** 블록이 날짜에 속하므로 "전체 블록"은 기간이 길어질수록 의미 없는 응답이 된다. 시간표 화면은 언제나 하루를 본다 (UR-08). 관리 화면에서 여러 날짜를 편집할 필요가 생기면 `from`/`to` 범위 질의를 추가하는 것이 확장 경로이며, 그때도 날짜 없는 전체 조회는 두지 않는다.

**블록이 없는 날짜의 응답은 빈 배열이다.** 오류가 아니다. 템플릿·반복이 후속이므로(UR-16.3) 사용자가 아직 그 날짜의 시간표를 만들지 않은 상태가 정상이며, 화면은 이를 "이 날짜의 시간표가 없음"으로 구분해 보여준다 (§6.2-2).
...
- 체크박스는 손가락으로 누를 수 있는 크기(**최소 48×48px** — §6.4의 터치 타겟 기준과 같은 값이며 정의처는 UR-20이다). 탭 한 번에 `PLANNED → DONE` (§3.2의 주 경로).
...
- 데이터 소스: `GET /api/schedule/today`. **언제나 하나의 날짜를 본다** (UR-16.2).
- 세로 타임라인. 마커(`endMinute == null`)는 얇은 선, 범위 블록은 높이가 지속 시간에 비례하는 상자 (F11).
- **현재 시각 블록을 강조하고 스크롤을 그 위치로 보낸다.** D15의 겹침 금지 덕분에 **그 날짜 안에서** 이 블록은 항상 최대 1개다.
- 각 블록 안에 매칭된 과제를 체크박스와 함께 표시 (D11). 매칭이 없는 블록(수학, 점심)은 라벨만 (F15, F16).
- 같은 과제가 두 블록에 나타나는 것은 정상이다 (F14). 한쪽에서 체크하면 양쪽 모두 반영된다.
- **그 날짜에 블록이 하나도 없으면 "이 날짜의 시간표가 없음"을 명시적으로 보여준다.** 빈 화면으로 두지 않는다 — F7에서 계획 없는 날을 명시하기로 한 것과 같은 이유다. 템플릿·반복 기능이 후속이므로(UR-16.3, NR-03) 이 상태는 오류가 아니라 흔한 정상 상태다.
...
- **책 관리 (UR-23)** — 목록에 제목·언어·단위·**마지막 계획 지점**(`GET /api/books/:id/progress`)을 보여준다. 부모가 "이 책 어디까지 계획했지"를 확인하는 곳.
  - 추가: 제목·언어·진도 단위 입력
  - 수정: 오타 정정, 단위 변경
...
> **아이 화면과 부모 관리 화면은 `UI 모드`의 구분이지 권한의 구분이 아니다** (UR-19, UR-19.1).
>
> - 두 화면은 **사용자 경험과 기능 목적**이 다르다. 아이 화면은 체크만 하면 되고, 관리 화면은 위험한 동작을 모아 둔다.
> - **MVP는 계정 인증·로그인·사용자별 권한 분리를 요구하지 않는다** (NR-04). 경로를 아는 사람은 관리 화면에 들어갈 수 있다.
> - 따라서 아래의 "아이 화면에 삭제를 두지 않는다"는 **권한 통제가 아니라 사고 방지**다. 인증·권한은 후속 요구사항이며 §8에 확장 경로가 있다.
...
아이의 기기는 **태블릿**이고 **하루 종일 켜져 있다** (UR-21). 이 두 가지가 레이아웃 기준을 정한다.

> **1024px은 주요 인수 테스트 기준이지 전용 폭이 아니다** (UR-20.1, UR-20.2).
>
> - 태블릿 가로 1024px을 **주요 검증 화면**으로 삼는다 — E2E가 이 폭에서 돈다 (T02).
> - **제품을 1024px 전용으로 제한하지 않는다.** 일반 데스크톱 브라우저에서도 모든 기능을 쓸 수 있어야 한다.
> - **모바일 세로 화면 최적화는 이번 MVP 필수 범위가 아니다** (NR-05). 깨지지 않는 것이 목표이고, 최적화는 후속이다.

| 항목 | 기준 | 이유 |
|---|---|---|
| 기준 폭 | **가로 1024px을 주요 인수 기준으로**, 세로 768px도 깨지지 않을 것. **더 넓은 데스크톱 폭에서도 동작해야 한다** (UR-20.2) | 태블릿은 거치해두고 쓰므로 가로가 기본이다. 다만 폭을 제한하지는 않는다 |
| 터치 타겟 | 체크박스·버튼 **최소 48×48px**(너비·높이 **둘 다** 48px 이상), 간격 8px 이상 | 손가락 조작. 초등학생은 정밀도가 낮다 |
| hover 의존 | **금지.** hover로만 드러나는 기능을 두지 않는다 | 터치 기기에 hover가 없다 |
| 본문 글자 | 최소 16px, 과제 제목 18px 이상 | 거치된 태블릿은 눈에서 멀다 |
| 가로 배치 | 가로 모드에서 **오늘 목록과 시간표를 2단으로** 병치 | 하루 종일 보는 화면에서 화면 전환 없이 둘 다 보이는 것이 상시 실행 전제(UR-21)에 맞는다 |
| 스크롤 | 오늘 화면은 스크롤 없이 한 화면에 들어가는 것을 목표로 | 아이가 스크롤해서 찾게 하지 않는다 |
| 화면 꺼짐 | 별도 처리 없음. 깨어나면 `visibilitychange`로 자동 갱신 (D16) | |

**터치 영역 값의 정의처는 UR-20 하나다 (R2-01).** 이 문서와 후속 화면 스펙 어디에서도 **화면별로 다른 값(44×44px 등)을 쓰지 않는다.** §6.2의 화면별 기술도 같은 `48×48px`을 쓴다. 후속 화면 태스크(T14 등)의 인수 기준과 UI/E2E 검사도 이 값을 쓴다 — 값이 갈리면 구현자가 화면마다 다른 판단을 하게 된다.
...
> **T02의 환경 스모크 테스트(`tests/env.test.ts`)는 아래 표의 어떤 항목도 대신하지 않는다.** 그 파일은 Node의 ICU·타임존 데이터와 Node 버전 정책만 확인하며 애플리케이션 함수를 import 하지 않는다. 아래 항목은 각 도메인 태스크의 완료 조건이다 (DR-04).

| 대상 | 케이스 |
|---|---|
| `getToday(now, tz)` | KST 00:01 / KST 23:59 / **프로세스 TZ가 UTC일 때와 `Asia/Seoul`일 때 동일 결과** (D8). 이 마지막 항목은 **별도 프로세스 두 개**를 `TZ=UTC`와 `TZ=Asia/Seoul`로 각각 실행해 검증한다 — 한 프로세스 안에서 `process.env.TZ`를 바꾸는 방식으로 대체하지 않는다. Node는 첫 `Date` 사용 시점에 TZ를 캐시할 수 있어 런타임 변경이 반영된다는 보장이 없다 |
...
| `POST /assignments/bulk-status` | **부록 C.3의 최소 테스트 조건 9건** (D22, R3-01) — 전부 유효 시 전부 변경 / 금지된 전이 1건이면 409·전부 롤백(UR-25.4) / 동일 상태만이면 성공·변경 0건 / 혼합이면 전체 성공(UR-25.5) / 반복 실행 시 최종 상태 동일 / 빈 날짜는 200 빈 batch / 요청 검증 실패는 400·write 0건 / 내부 실패는 500·전부 롤백 / test-only seam으로 첫 update 이후 실패시켜 전후 스냅샷 deep equal. **담당은 T09이며 배치 1 범위 밖이다** |
...
| 항목 | 제외 이유 | 확장 경로 |
|---|---|---|
| 인증 / 로그인 / 사용자별 권한 | **MVP 필수가 아니다** (UR-19.1, NR-04). 아이의 진입 마찰을 늘린다. **후속 요구사항으로 둔다** — 배제가 아니라 보류다 | 화면 모드 구분은 이미 있으므로 인증 계층만 앞에 붙인다 |
| 다중 사용자 / 다중 가정 | 초기 목표는 **Mac mini에서 도는 단일 가족용 시스템**이다 (UR-14, NR-09) | `Child` 테이블 + 모든 테이블에 `childId` FK |
| **시간표 템플릿 · 반복 시간표** | UR-16.3 — 같은 시간표를 여러 날짜에 재사용하는 기능은 **현재 필수 요구사항이 아니다** (NR-03). 날짜별로 블록을 만든다 | `ScheduleTemplate` 테이블 + `ScheduleBlock.templateId` nullable. **기존 데이터 마이그레이션이 필요 없다** — 블록이 이미 날짜를 갖는다 |
| 다른 DB로의 전환 | 가능성은 열어두되 **이번 MVP에서 다루지 않는다** (UR-14.1, NR-07) | Prisma의 provider 교체 + 마이그레이션 재생성 |
| 모바일 세로 화면 최적화 | **이번 MVP 필수 범위가 아니다** (UR-20, NR-05). 깨지지 않는 것까지가 목표다 | 반응형 레이아웃 추가 |
| 실제 진도(`actualEndUnit`) | 실물은 계획만 기록한다. D6에서 계획 기반을 명시적으로 택했다 | `Assignment.actualEndUnit` 추가. **계획 체인은 계획끼리 유지**하고 실적은 별도 표시 |
| ~~방학 기간 개념~~ | **v1에 포함됨** — D18로 편입 | — |
| ~~방학 기간을 화면에서 편집~~ | **더 이상 단순 제외가 아니다** (UR-15.2). 제품은 방학 계획 기간을 **변경하거나 새 기간을 만들 수 있는 구조**여야 한다. **파일럿에서는 코드 상수를 쓰지만 그것이 제품의 영구 고정 기간이 되어서는 안 된다** (UR-15.4). 편집 UI의 구현 시점은 후속 배치이며 부록 A.3의 "기간 관리" 항목이다 | `Settings` 또는 `Term` 테이블로 승격. D18이 그 경로를 규정한다 |
| 실시간 푸시(WebSocket/SSE) | **이번 범위에 추가하지 않는다** (UR-21, NR-06). 기기 2~3대, 변경 빈도가 낮아 폴링으로 충분하다 | SSE 추가 (폴링 코드를 교체) |
...
| 알림 / 푸시 | 요구되지 않았다 (NR-08). 화면이 상시 열려 있으므로(UR-21) 알림의 이득이 작다 | — |
...
> **태스크 번호와 범위의 유일한 권위는 `docs/specs/T##-*.md`다.** 이 부록은 그 스펙들을 요약하고, 아직 스펙이 없는 후속 범위를 기록한다.
>
> **태스크 개수를 여기에 적지 않는다.** 분해는 여러 차례 조정되었고, 개수를 본문에 적으면 조정될 때마다 갈라진다 (§2 결정 개수 표기를 없앤 것과 같은 이유).

### A.1 배치 1 — 확정 (스펙 존재)

아래 네 태스크만 착수 가능한 상태다. 명칭·범위·의존은 각 스펙 파일과 일치하며, **차이가 있으면 스펙이 옳다.**

| # | 태스크 | 스펙 | 범위 | 의존 |
|---|---|---|---|---|
| **T01** | 프로젝트 스캐폴드 | [`T01-scaffold.md`](./specs/T01-scaffold.md) | Next.js App Router + TS strict + Tailwind v4 + ESLint 9 flat config. `.nvmrc = 22`(CI 기준선)와 `engines.node = "22 \|\| 24 \|\| 26"`(허용 집합) 고정 (D21), `next build`+`next start` 실행 스크립트 (D16-e), `next typegen`을 포함한 typecheck | – |
| **T02** | 테스트 하니스 | [`T02-test-harness.md`](./specs/T02-test-harness.md) | vitest + Playwright + CI 전체 교체. 실행 환경 전제(ICU·타임존·Node 정책) 검증. **도메인 규칙은 검증하지 않는다** | T01 |
| **T03** | Prisma 스키마 · 마이그레이션 기반 | [`T03-prisma-schema.md`](./specs/T03-prisma-schema.md) | §1.2 스키마(enum 포함, `prisma`/`@prisma/client` `^6.2.0` 이상 — D13), 초기 마이그레이션, WAL 클라이언트 (D16-e), 연결 문자열 정책 (D19), **DB 구조 기계 검증**, 테스트 DB 헬퍼 | T02 |
| **T04** | 초기 시드 | [`T04-seed.md`](./specs/T04-seed.md) | 부록 C의 시드(책 9 · 블록 10 · 과제 27)를 단일 트랜잭션으로. 기존 사용자 데이터 비파괴, 표 전체 exact match 검증 | T03 |

**구현 순서는 `T01 → T02 → T03 → T04`로 고정이다** (Codex 스펙 검증 DR-05·DR-08).

- 네 태스크가 `package.json`·`package-lock.json`·`.gitignore`·`.github/workflows/ci.yml`을 공유하므로 **병렬 구현·병렬 PR을 허용하지 않는다.**
- 각 태스크의 브랜치는 직전 태스크가 `main`에 Merge된 뒤에 만든다.
- 각 스펙의 "구현 순서상의 위치" 절이 이 규칙을 반복해 명시한다.

시드를 스키마와 분리한 이유(원래 한 태스크였다): 시드는 부록 C의 표를 옮기는 **데이터 작업**이라 리뷰 관점이 다르고, 구조 검증과 데이터 검증이 서로를 가리지 않아야 한다.

### A.2 배치 1 스펙이 이미 예약한 번호 (스펙 미작성)

**이 표는 새 번호를 배정하는 것이 아니라, 배치 1 스펙 본문이 이미 참조하고 있는 번호를 기록한 것이다.** 후속 배치를 분해할 때 이 예약을 지켜야 하며, 바꾸려면 아래 "참조 위치"의 스펙을 함께 고쳐야 한다.

| 예약 번호 | 범위 | 참조 위치 |
|---|---|---|
| **T05** | 도메인 기반 — enums 재수출 · clock · term · 상태 전이. **D8의 2프로세스 검증(§7.1)이 이 태스크의 완료 조건이다** | T02 스펙 "이 태스크가 검증하지 않는 것", T03 금지사항 |
| **T07** | 검증 스키마 (Zod) — I1~I12, 판별 유니온. enum 값 범위 보장의 실제 담당 | T03 요구사항 3, 스펙 미정 9 |
| **T09** | API — 과제. `E2E_TEST_MODE` / `x-e2e-now`를 읽는 코드가 여기서 생긴다 (D20) | T02 `playwright.config.ts` 주석, 스펙 미정 5 |
| **T12** | API — 대시보드. 요청 단위 단일 `today` 확정 경로 (§5.1) | T02 "이 태스크가 검증하지 않는 것" |
| **T14** | 화면 — 오늘. `src/app/page.tsx` 임시 화면과 `e2e/smoke.spec.ts`를 대체한다 | T01 요구사항 12, T02 스펙 미정 10 |
| **T17** | 화면 — 날짜 상세. 부록 C.3의 일괄 완료(`bulk-status`) 진입점이 여기 들어간다. **UR-25.1·UR-25.2·UR-25.4·UR-25.5가 그 스펙의 필수 조건이다** — 부모용 화면에만 둘 것, 단일 전이 규칙을 우회하지 않을 것, 원자적 전부 롤백과 동일 상태 멱등 no-op을 그대로 반영할 것 (**D22**, 부록 C.3) | T04 금지사항, 스펙 미정 1 |
| **T19** | 마무리 — 실행 문서(`README.md`) · CI · 문서 동기화 | T01 금지사항 |

### A.3 후속 배치 — 범위 목록 (ID 미정)

아래는 **스펙이 없으므로 태스크 ID를 확정하지 않는다.** 순서는 의존 방향이며 태스크 경계와 같지 않다. A.2에 예약 번호가 있는 항목만 번호를 병기한다.

| 범위 | 설계 근거 | 예약 번호 |
|---|---|---|
| 도메인 기반 — enums 재수출 · clock · term · 상태 전이 | §1.4, D8, D18, §3.2. 전부 순수 함수 + 단위 테스트 | T05 |
| 도메인 — 진도 해석 | §4.1 해석 규칙 + §4.4의 9케이스 (DB 없이) | 미정 |
| 검증 스키마 (Zod) | I1~I12, 판별 유니온 | T07 |
| 서비스 계층 — 과제 | §4.2 체인 조회 + §4.3 배치 로딩 | 미정 |
| API — 과제 · 상태 전이 · reorder · bulk · bulk-status | §5.2, 부록 C.3, **D22**, D20. **`bulk-status`의 최소 테스트 조건 9건이 이 태스크의 필수 완료 조건이다** | T09 |
| API — 책 (보관 포함) | §5.3, D17 (find-or-create, 보관, 삭제 제약) | 미정 |
| API — 시간표 (겹침 검증 + `serverNowMinute`) | §5.4, D15, D16-d | 미정 |
| API — 대시보드 · 달력 | §5.1, §5.5 (단일 `today` 보장) | T12 (대시보드) |
| 갱신 계층 — 폴링 · 가시성 · 날짜 전환 · 시각 오프셋 | D16 (a)~(d). 화면과 분리된 재사용 훅으로 | 미정 |
| 화면 — 오늘 | §6.2의 1, §6.4 | T14 |
| 화면 — 시간표 (태블릿 2단 병치) | §6.2의 2, §6.4 | 미정 |
| 화면 — 달력 | §6.2의 3 | 미정 |
| 화면 — 날짜 상세 (일괄 완료 포함) | §6.2의 4, 부록 C.3 | T17 |
| 화면 — 관리 (책 · 시간표) | §6.2의 5, D17 | 미정 |
| **기간 관리 — 방학 계획 기간의 변경·추가** | **UR-15.2 · UR-15.4.** D18의 코드 상수는 파일럿 기본값이며 제품의 영구 고정 기간이 아니다 | 미정 |
| **배포용 초기화 — 파일럿 시드와 분리된 일반 배포 초기화 명령** | **UR-24.2 · UR-24.4.** 운영 배포에서 사용자 확인 없이 개인 숙제 데이터가 주입되어서는 안 된다 | 미정 |
| 마무리 — 실행 문서 · CI · 문서 동기화 | — | T19 |

**위 두 항목은 새 태스크 번호를 만들지 않는다** (UR-16.4). 후속 배치를 분해할 때 A.2의 예약 번호 규칙에 따라 번호가 붙는다.

**갱신 계층을 화면 범위에 섞지 않는다.** D16의 규칙(특히 날짜 전환 감지와 시각 오프셋)은 **DOM 없이 단위 테스트 가능한 로직**이다. 화면에 묻으면 테스트가 E2E로만 가능해진다.

**E2E(§7.3)는 각 화면 범위에 붙인다.** 별도 태스크로 몰지 않는다 — 몰면 마지막에 밀려서 안 쓰이게 된다.
...
> **이 시드는 파일럿·개발·테스트용이다** (UR-24, UR-24.1). **일반 운영 배포에서 사용자 확인 없이 개인 숙제 데이터가 자동 주입되어서는 안 된다** (UR-24.2). 현재 `db:setup`은 **파일럿·개발 초기화 명령**이며(UR-24.3), 일반 배포용 초기화 명령과의 분리는 후속 범위다 (UR-24.4, 부록 A.3).
>
> **날짜의 연도 `2026`은 mockup에서 읽은 값이 아니다.** 사진에는 월·일만 있다 (MR-02). `2026`은 **이번 파일럿 데이터에 사용자가 지정한 연도**이며(UR-15.3), 제품 전체가 이 기간으로 고정되는 것을 뜻하지 않는다 (UR-15.2).

### C.1 시간표

D11의 표를 그대로 시드한다. 실물(`daily-schedule.jpg`)과 1:1 대응한다.

**블록은 날짜를 갖는다** (UR-16). `daily-schedule.jpg`에는 날짜가 없으므로(MR-20), 시드는 **파일럿 기간의 첫날 `2026-07-29` 하루분 10건**을 넣는다.

- 10건이라는 수는 UR-24.5가 고정한다. 20일 전체에 넣으면 200건이 되어 그 조건과 충돌한다.
- **결과: 파일럿 시드 직후 `2026-07-29` 외의 19일은 시간표가 비어 있다.** 이는 오류가 아니다 — 시간표 템플릿·반복은 현재 요구가 아니므로(UR-16.3, NR-03) 사용자가 날짜별로 만든다. 화면은 이 상태를 "이 날짜의 시간표가 없음"으로 표시한다 (§6.2-2).
- **날짜 `2026-07-29`는 사용자가 지정한 값이다** (UR-26). `daily-schedule.jpg`에서 판독한 날짜가 아니라, **파일럿 기간의 첫 날짜를 대표 일일 시간표 날짜로 명시적으로 지정**한 것이다. `2026`이라는 연도에 대한 UR-15.3과 같은 성질이며, **제품 전체에 하드코딩되는 불변값이 아니다** (UR-26.6).
- **다른 19일에 시간표를 자동 복제하지 않는다** (UR-26.2). 사용자가 날짜별로 입력한다 (UR-26.5).

### C.2 과제 계획

§0.1의 표를 그대로 시드하되, **다음 두 가지를 지킨다.**

1. **제목이 불명확한 항목은 시드하지 않는다.** 7/29의 한글책은 사진에서 판독이 안 된다 (MR-12). 빈 상태로 두고 사용자가 관리 화면에서 추가한다 — UR-24.6, NR-02, CLAUDE.md의 "추측으로 진행하지 않는다"에 해당한다.
...
**파일럿 착수 예시의 기준일을 `2026-08-01`로 고정한다** (R2-05). 아래 숫자는 **그 고정 기준일에서 나온 예시**이며, 실행 시각에 따라 변하는 "오늘"을 뜻하지 않는다. `today`는 언제나 D8이 확정하고 `OVERDUE`는 저장하지 않는다 (D7, UR-18).

- 방학(파일럿 기간)은 `2026-07-29`에 시작한다 (D18).
- 기준일 `2026-08-01`에는 **7/29 · 7/30 · 7/31 사흘이 이미 지나 있다.**
- 밀린 것의 정의는 §3.4와 같다: `date < today && status ∈ {PLANNED, IN_PROGRESS}`. **완료(`DONE`)·건너뜀(`SKIPPED`)은 제외한다.**
- 시드 과제는 전부 `PLANNED`이므로(부록 C.2, T04) 지난 사흘의 시드 과제가 그대로 밀린 것이 된다.

| 지난 날짜 | 시드 과제 수 (T04 요구사항 10) |
|---|---:|
| 2026-07-29 | 2 (Big Note ch.6, 일기 — 판독 불가 한글책은 시드하지 않는다, MR-12) |
| 2026-07-30 | 2 (Big Note ch.12, work sheet) |
| 2026-07-31 | 2 (엄마 5분만, 무지개 물고기) |
| **합계** | **6** |

즉 기준일 `2026-08-01`에 앱을 처음 켜면 **밀린 숙제가 6건**이다. 실제로는 종이 계획표에서 이미 수행했을 수 있다. 기준일을 `2026-07-31`로 잡으면 지난 날짜는 이틀이고 밀린 것은 4건이 된다 — **숫자와 기준일은 항상 함께 움직인다.**

따라서 **날짜 상세 화면(§6.2-4)에 "이 날 전부 완료 처리" 동작이 필요하다.** 첫 실행 때 부모가 지난 날짜를 정리하는 유일한 경로이며, 이것이 없으면 6건을 하나씩 눌러야 한다.

- 구현: `POST /api/assignments/bulk-status` `{ "date": "2026-07-29", "to": "DONE" }` (UR-25)
- 이 동작은 **관리·상세 화면에만** 둔다 (UR-25.1). 아이 화면에 두면 전부 완료 처리로 숙제를 없앨 수 있다 (§6.3).
- **`bulk-status`는 단일 과제 전이 규칙(§3.2)을 우회하지 않는다** (UR-25.2). 각 항목이 같은 검증을 통과해야 한다.
- 적용 대상은 **요청에 지정된 하나의 `date`에 속한 과제 전체**다. MVP에는 인증·권한 분리가 없으므로(UR-19.1, NR-04) 이 동작의 경계는 권한이 아니라 **화면 배치(UR-25.1)와 요청 단위 검증**이다.

**일부 실패 처리 정책 — 결정 완료 (D22, `OPEN-02` 2026-08-01 사용자 승인).**

| 규칙 | 내용 | 요구 ID |
|---|---|---|
| **원자성** | **전부 성공하거나 전부 롤백.** 실제 실패 항목이 1건이라도 있으면 요청 전체를 롤백한다. **부분 성공은 없다.** 쓰기는 단일 트랜잭션이다 | **UR-25.4** |
| **실제 실패의 정의** | 존재하지 않는 과제 · 허용되지 않는 상태 전이(§3.2) · 권한 오류 · 검증 오류. **현재 `{ date, to }` 계약에서의 사상은 §5.2에 있다** — 검증 오류는 요청 수준 400, 허용되지 않는 전이는 409, 존재하지 않는 과제와 권한 오류는 발생 불가 (R3-01) | **UR-25.4** |
| **동일 상태 재적용** | **성공하는 멱등 no-op.** 데이터를 바꾸지 않고 성공으로 집계하며 **실패로 세지 않는다.** 이미 `DONE`인 과제에 `DONE`을 보내도 전체 롤백이 일어나지 않는다 | **UR-25.5** |

이 조합이 위 시나리오를 성립시킨다 — 지난 날짜에는 **이미 완료된 과제가 섞여 있는 것이 정상**이므로, 동일 상태를 실패로 두면 원자성과 만나 매번 전체 롤백이 된다 (D22의 기각한 대안).

**최소 테스트 조건 (구현 태스크의 필수 완료 조건, D22·R3-01).** 아래 9건은 `bulk-status`를 구현하는 태스크가 **반드시** 검증한다. 담당은 **엔드포인트 T09 · 진입 화면 T17**(둘 다 예약 번호)이며 **배치 1(T01~T04)의 범위 밖**이다.

| # | 조건 | 기대 |
|---|---|---|
| 1 | 모든 항목이 유효하다 | **전부 변경**되고 커밋된다. `changed`가 대상 수와 같다 |
| 2 | 대상 중 **1건 이상이 금지된 전이**다 | **HTTP 409, 전부 롤백.** 어떤 Assignment도 바뀌지 않고 `applied: false`, `failures[]`에 **실재하는 대상의 id**·현재 상태·요청 상태·사유가 담긴다 (UR-25.4) |
| 3 | **동일 상태 항목만** 포함한다 (전부 이미 `DONE`) | **성공.** 데이터 변경 0건, `changed: 0`, `unchanged`가 대상 수와 같다 (UR-25.5) |
| 4 | 동일 상태 항목과 **정상 변경 항목이 섞여 있다** | **요청 전체 성공.** 정상 변경만 적용되고 동일 상태 항목은 그대로다. 롤백이 일어나지 않는다 (UR-25.5) |
| 5 | **같은 요청을 반복 실행**한다 | 최종 상태가 동일하다(멱등). 2회차는 전부 `unchanged`로 성공한다 |
| 6 | **빈 날짜** — 유효한 날짜지만 대상이 0건 | **HTTP 200**, `changed: 0`, `unchanged: 0`, `items: []`. 롤백도 오류도 아니며 반복 실행해도 같은 결과다 |
| 7 | **요청 검증 실패** — 잘못된 날짜 형식 / 허용되지 않는 `to` / 필수 필드 누락 | **HTTP 400**, `BULK_STATUS_INVALID_REQUEST`, `applied: false`. **트랜잭션 write 0건**이고 `failures`에 Assignment ID를 만들지 않는다 |
| 8 | **내부 실패** — 트랜잭션 안에서 예상하지 못한 오류 | **HTTP 500**, `BULK_STATUS_INTERNAL_ERROR`, `applied: false`, 전부 롤백. 내부 예외 세부정보를 응답에 담지 않는다 |
| 9 | **결정적 rollback** — 트랜잭션 안에서 **첫 update가 성공한 직후** test-only seam이 오류를 던진다 | **요청 전 전체 대상 스냅샷과 실패 후 스냅샷이 deep equal**이다. 첫 항목의 상태·`completedAt`·`updatedAt`이 되돌아온다 |

**2·9번의 중복과 충돌을 정리한다.** 2번은 **응답 계약**(409·`applied:false`·`failures`)을, 9번은 **쓰기 원자성**(전후 스냅샷 deep equal)을 검증한다. 둘을 한 테스트에 합치지 않는다.

**9번이 롤백을 반증 가능하게 만드는 방법 (R3-01, T04 요구사항 2-A·2-B와 같은 원칙).**

| 항목 | 값 |
|---|---|
| 실패 만드는 법 | 트랜잭션 안에서 **최소 한 건의 실제 update가 실행된 뒤** test-only seam이 오류를 던진다 |
| 스냅샷 | 그 날짜 대상 Assignment의 **전체 행·전체 컬럼**(`status`·`completedAt`·`updatedAt` 포함)을 안정 정렬해 캡처하고, 실패 후 **새 연결**로 다시 읽어 deep equal을 단정한다 |
| 검출되는 잘못된 구현 | 트랜잭션을 제거한 구현, 또는 첫 update를 트랜잭션 **밖으로** 옮긴 구현 — 첫 변경이 커밋된 채 남아 deep equality가 깨진다 |
| **쓰지 않는 것** | 존재하지 않는 Assignment ID(요청 모델상 지정 불가), 두 번째 writer 연결, SQLite writer lock / `SQLITE_BUSY`, 처리 순서에 결합된 단정 |
| seam의 운영 경로 | **노출하지 않는다.** 라우트 핸들러의 운영 호출 경로에서는 주입되지 않으며 환경변수 스위치도 두지 않는다 |

이 계약은 **후속 태스크 T09(API)·T17(화면)의 완료 조건**이며 **배치 1(T01~T04)의 구현 범위가 아니다.** 새 태스크 번호를 만들지 않는다.

---

## 사용자 확정 사항

**요구사항의 권위 있는 출처는 [`requirements.md`](./requirements.md)다.** 아래는 그 문서로 옮겨진 확정 이력의 색인이며, 값의 정의처가 아니다.

| 시점 | 내용 | 현재 위치 |
|---|---|---|
| 2026-07-31 | 방학 기간, 한글책 제목 불명확 처리, 아이 기기, 앱 실행 방식 | UR-15, MR-12·NR-02, UR-20, UR-21 |
| 2026-08-01 | 요구사항 원문 제공 | UR-01 ~ UR-13, OR-01 ~ OR-03 |
| 2026-08-01 | 설계 가정 12건(AP-01~AP-12) 결정 — **AP-03은 거절 후 정정** | UR-14 ~ UR-25, 조건 **31건**(이 시점 기준. `OPEN-02` 결정으로 2건이 더해져 현재 총 **33건** — 전체 목록은 [`requirements.md` §8.1-b](./requirements.md#81-b-정방향--조건-33건의-반영-위치)), NR-03 ~ NR-09 |
| 2026-08-01 | `OPEN-01` 결정 — 시드 시간표 블록의 날짜 | UR-26, UR-26.1 ~ UR-26.6 |
| 2026-08-01 | `OPEN-02` 결정 — `bulk-status`의 일부 실패 정책 (원자적 전부 롤백 + 동일 상태 멱등 no-op) | UR-25.4, UR-25.5, **D22** |

## 미해결 질문

**미해결 질문이 없다.** 요구 해석 수준의 질문도, 구현 세부 미결도 없다.

- `OPEN-01`(시드 시간표 블록의 날짜)은 2026-08-01에 사용자가 결정해 **UR-26** 계열로 승격되었다.
- `OPEN-02`(`bulk-status`의 일부 실패 정책)도 같은 날 사용자가 결정해 **UR-25.4·UR-25.5**로 승격되었고, 설계 결정은 **[D22](./decisions.md#d22)**, 계약은 §5.2, 최소 테스트 조건은 부록 C.3와 §7.2에 있다.

**구현 착수 가능 여부는 이 문서가 아니라 [`workflow/STATUS.md`](./workflow/STATUS.md)가 정한다.**
