...
| `orderIndex` | 같은 `date` 안에서의 표시 순서. 0부터 | F8. 유니크 제약을 걸지 않는다 → D8-b |
...
| I11 | `Book` 삭제 시 참조하는 `Assignment`가 있으면 거부 | 409 |
...
| 트리거 | 실패 모드 |
|---|---|
| 자정 크론 | 이 앱은 가정용 SQLite 앱이다. 노트북이 자거나 앱이 꺼져 있으면 **크론이 그냥 실행되지 않는다.** 며칠 안 열면 상태가 통째로 낡는다 |
| 조회 시 일괄 갱신(lazy sweep) | 어차피 조회 시점에 계산하면서 **쓰기까지 추가로 한다.** B보다 순수하게 나쁘다 |
| 쓰기 시 갱신 | 아무도 아무것도 쓰지 않을 때가 정확히 밀린 상황이다. **원리적으로 발동하지 않는다** |

즉 A는 **저장된 상태가 현실과 어긋나는 정합성 실패**를 구조적으로 안고 있다.

**A의 통상적 이점(인덱싱)이 여기서는 성립하지 않는다.** "밀림"은 `date`와 `status`의 순수 함수이고 둘 다 저장·인덱싱된 컬럼이다. `@@index([date, status])` 하나로 밀린 목록 질의(`date < ? AND status IN (...)`)와 오늘 목록 질의(`date = ?`)가 모두 커버된다. 파생 계산 때문에 인덱스를 못 쓰는 상황이 아니다.

**추가로 A는 상태 공간을 오염시킨다.** `OVERDUE`를 상태로 두면 `OVERDUE → DONE` 전이가 필요하고, 완료 후 날짜를 미래로 수정하면 `DONE → PLANNED`인지 `→ OVERDUE`인지 판단이 필요해진다. B에서는 이런 질문이 아예 생기지 않는다 — 저장 상태는 그대로고 파생값만 바뀐다.

**선택: B.** v-hw와 동일한 결론이며, 이 프로젝트에서도 근거가 성립한다.
...
**부수 결정 (D8-b):** `Assignment.orderIndex`에는 유니크 제약을 걸지 않는다. 유니크면 순서 변경 시 임시값 우회가 필요해진다. 대신 **정렬은 항상 `orderIndex ASC, id ASC`** 로 하여 동점에서도 결정적이게 하고, 순서 변경은 한 트랜잭션에서 전체 인덱스를 다시 쓴다.
...
`id`까지 비교에 넣는 이유: 같은 날짜·같은 `orderIndex`가 가능하므로(D8-b에서 유니크 제약을 걸지 않았다) 결정적 순서를 보장하기 위함이다.
...
| 메서드 | 경로 | 설명 |
|---|---|---|
| `GET` | `/api/books?q=&language=` | 목록/자동완성 (D4의 마찰 완화) |
| `POST` | `/api/books` | 생성. `title` 중복 시 **409가 아니라 기존 책을 반환**(find-or-create) |
| `PATCH` | `/api/books/:id` | 수정 |
| `DELETE` | `/api/books/:id` | 삭제. 참조하는 과제가 있으면 **409** (I11) |
...
- 책 목록: 제목, 언어, 단위, **마지막 계획 지점** (`GET /api/books/:id/progress`). 부모가 "이 책 어디까지 계획했지"를 확인하는 곳.
...
| `DELETE /books/:id` | 참조 있으면 409 (I11) |
...
**시간 고정:** E2E는 반드시 시각을 고정해서 돌린다 (`APP_TIMEZONE` + 고정 `now` 주입). 실제 시계에 의존하면 자정 근처나 CI 타임존에서 무작위로 깨진다.
...
| 방학 기간(시작/종료일) 개념 | "D-N", "전체 진행률"은 있으면 좋지만 어떤 핵심 기능도 이것을 요구하지 않는다 | 상수 2개 또는 `Settings` 단일 행 |
...
| # | 태스크 | 범위 | 의존 |
|---|---|---|---|
| T01 | 프로젝트 초기화 | Next.js + TS + vitest + Playwright + lint. **`prisma`/`@prisma/client`를 `^6.2.0` 이상으로 (D13)**. CI가 실제로 도는 상태까지 | – |
| T02 | Prisma 스키마 · 마이그레이션 · 시드 | §1.2 스키마(enum 포함), D11의 시간표 시드 | T01 |
| T03 | 도메인 기반 — enums 재수출 · clock · 전이 | §1.4, D8, §3.2. **전부 순수 함수 + 단위 테스트** | T02 |
| T04 | 도메인 — 진도 해석 | §4.1 해석 규칙 + §4.4 테스트 (DB 없이) | T03 |
| T05 | 검증 스키마 (Zod) | I1~I8, 판별 유니온 | T03 |
| T06 | 서비스 계층 — 과제 CRUD | §4.2 체인 조회 + §4.3 배치 로딩 포함 | T02, T04, T05 |
| T07 | API — 과제 · 상태 전이 · reorder · bulk | §5.2 | T06 |
| T08 | API — 책 | §5.3 (find-or-create, 삭제 제약) | T06 |
| T09 | API — 시간표 (겹침 검증 포함) | §5.4, D15 | T02, T05 |
| T10 | API — 대시보드 · 달력 | §5.1, §5.5 (단일 `today` 보장) | T06 |
| T11 | 화면 — 오늘 · 시간표 (아이용) | §6.2의 1, 2 | T09, T10 |
| T12 | 화면 — 달력 · 날짜 상세 · 관리 (부모용) | §6.2의 3, 4, 5 | T07, T08, T09 |

E2E(§7.3)는 T11·T12에 각각 붙인다. 별도 태스크로 몰지 않는다 — 몰면 마지막에 밀려서 안 쓰이게 된다.
...
7. `any`를 쓰지 않는다. 타입이 불분명하면 멈추고 질문한다. (CLAUDE.md)
8. 스펙(`docs/specs/T##-*.md`)에 없는 판단이 필요하면 진행하지 말고 질문한다. (CLAUDE.md)

---

## 미해결 질문 (사용자 확인 필요)

구현 착수 전에 답이 필요하다. 임의로 정하지 않았다.

| # | 질문 | 기본안 |
|---|---|---|
| Q1 | 방학 기간이 실물대로 **7/29~8/17**인가? 달력의 기본 표시 범위에 영향 | 실물 그대로 가정 |
| Q2 | `7/29` 칸의 한글책 제목이 사진에서 불명확하다 (`김치찌…`). 정확한 제목은? | 시드에서 제외하고 사용자가 입력 |
| Q3 | 아이가 쓸 기기는 무엇인가 (부모 폰 / 태블릿 / PC)? 오늘·시간표 화면의 터치 타겟과 레이아웃 기준이 달라진다 | 모바일 우선 |
| Q4 | 앱이 어디서 도는가 (집 PC 상시 / 필요할 때만 실행 / 배포)? D7에서 크론을 기각한 근거와 직결된다 | 상시 실행 아님으로 가정 |
