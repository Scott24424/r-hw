...
- 상태: **승인됨** (2026-08-01). 설계 가정 12건(AP-01~AP-12)에 대한 사용자 결정이 반영되었다 — [`requirements.md` §5](./requirements.md#5-ap-01--ap-12-처리-결과)
...
| **T17** | 화면 — 날짜 상세. 부록 C.3의 일괄 완료(`bulk-status`)가 여기 들어간다. **UR-25.2·UR-25.3이 그 스펙의 필수 조건이다** — 단일 전이 규칙을 우회하지 않을 것, 일부 실패 시 처리 방식을 결정적으로 정의할 것 | T04 금지사항, 스펙 미정 1 |
...
| API — 과제 · 상태 전이 · reorder · bulk · bulk-status | §5.2, 부록 C.3, D20 | T09 |
...
- **일부 항목이 실패했을 때의 처리 방식을 그 태스크의 명세와 테스트에서 결정적으로 정의한다** (UR-25.3). "전부 성공하거나 전부 롤백"인지 "성공분만 반영하고 실패 목록을 반환"인지를 스펙이 고르며, 구현자가 고르지 않는다. 담당은 **T17**(예약 번호)이다.
...
| 2026-08-01 | 설계 가정 12건(AP-01~AP-12) 결정 — **AP-03은 거절 후 정정** | UR-14 ~ UR-25, 조건 22건, NR-03 ~ NR-09 |
| 2026-08-01 | `OPEN-01` 결정 — 시드 시간표 블록의 날짜 | UR-26, UR-26.1 ~ UR-26.6 |

## 미해결 질문

**미해결 질문이 없다.** 요구 해석 수준의 질문도, 구현 세부 미결도 없다. 마지막까지 남아 있던 `OPEN-01`(시드 시간표 블록의 날짜)은 2026-08-01에 사용자가 결정해 **UR-26**으로 승격되었다.
