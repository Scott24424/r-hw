...
- 함께 볼 것: [`decisions.md`](./decisions.md) — 각 설계 결정의 근거와 기각한 대안 (결정 목록은 §2의 표가 유일한 색인이다)
...
| [D21](./decisions.md#d21) | Node 22를 최소·CI 기준 버전으로 고정 (`.nvmrc`가 정의처) | §7.1, §7.3 |
...
| # | 태스크 | 스펙 | 범위 | 의존 |
|---|---|---|---|---|
| **T01** | 프로젝트 스캐폴드 | [`T01-scaffold.md`](./specs/T01-scaffold.md) | Next.js App Router + TS strict + Tailwind v4 + ESLint. `.nvmrc`로 Node 22 고정 (D21), `next build`+`next start` 실행 스크립트 (D16-e), `next typegen`을 포함한 typecheck | – |
...
| **T17** | 화면 — 날짜 상세. 부록 C.3의 일괄 완료가 여기 들어간다 | T04 금지사항, 스펙 미정 1 |
