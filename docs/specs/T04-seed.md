...
|---|---|---|---|
| `beforeCreateHook` | 삭제 직후, 생성 전 | `deleteMany`가 트랜잭션 밖에 있음, **또는 트랜잭션 경계가 1단계(Book upsert) 뒤에서 시작함** | 위반 케이스 18 · 18c (경계 mutation은 29b) |
...
| timestamp | `Date`는 `toISOString()`, 정수 epoch는 그대로 둔다. **값을 버리거나 반올림하지 않는다** — 기존 `Big Note` 한 행의 `updatedAt` 차이가 29b mutation을 검출하는 신호 중 하나다 |
...
| 29b | **트랜잭션이 1단계부터 시작하는가** (R2-04) | 트랜잭션 경계를 **2단계로 미룬다.** 1단계(Book 9권 upsert)를 `client`로 먼저 실행해 커밋하고, `$transaction(...)`은 2단계부터 감싼다 | `force 중 실패하면 삭제가 롤백된다`(18)와 `블록 생성 후 실패하면 force가 지운 사용자 데이터가 복원된다`(18c)가 **실패한다.** **확인 후 되돌린다** |
| 29c | **트랜잭션이 4단계를 포함하는가** (R2-04 Round 4) | 트랜잭션 경계를 **5단계로 미룬다.** 1~4단계(Book upsert → force 삭제 → `beforeCreateHook` → 블록 10건 생성)를 `client`로 순차 실행해 커밋하고, `$transaction(...)`은 5단계부터 감싼다 | `블록 생성 도중 실패하면 새 DB에 아무 행도 남지 않는다`(18b)와 `블록 생성 후 실패하면 force가 지운 사용자 데이터가 복원된다`(18c)가 **둘 다** 실패한다. **확인 후 되돌린다** |
| 29d | **트랜잭션이 6단계를 포함하는가** (R2-04 Round 4) | 트랜잭션 경계를 **7단계로 미룬다.** 1~6단계를 `client`로 순차 실행해 커밋하고, `$transaction(...)`은 7단계(`afterAssignmentsHook`)만 감싼다 | `과제 생성 후 실패하면 블록과 과제가 모두 롤백된다`(18d)가 실패한다. **확인 후 되돌린다** |
| 29e | **트랜잭션 경계 자체가 있는가** (R2-04) | `$transaction(async (tx) => { ... })` 래퍼를 **완전히 제거**하고 1~7단계를 `client` 위에서 순차 실행한다 (hook 호출 위치는 그대로) | 롤백 테스트 **4건 전부**(18·18b·18c·18d)가 실패한다. **확인 후 되돌린다** |

**29b~29e는 "단계를 옮기는" mutation이 아니라 "트랜잭션 경계를 미루는" mutation이다 (R2-04 Round 4).** Round 4가 지적한 결함은 이것이었다 — 조건부 생성 단계(4·6단계)만 `$transaction` **앞으로 끌어올리면**, `force` 삭제가 아직 실행되지 않은 시점이라 `count() === 0` 가드가 **거짓**이 되어 생성이 통째로 skip된다. 그러면 트랜잭션 밖에 남는 행이 없고, 트랜잭션 안의 삭제가 hook 예외로 롤백되어 전후 스냅샷이 같아지므로 **잘못된 구현이 18c·18d를 통과한다.** 6단계만 앞으로 옮기면 Book 9권 upsert(1단계)보다 먼저 실행되어 `Assignment.bookId` 외래키 전제도 깨진다.

**경계를 미루는 방식은 그 두 문제를 동시에 없앤다.** 단계의 **순서와 선행 조건은 원본 그대로**이고 어느 단계가 커밋되는지만 달라진다 — 1~N단계는 `client`에서 이미 커밋되고, 트랜잭션은 N+1단계부터 시작한다. 따라서 `force` 삭제는 생성 가드보다 **먼저** 실행되고, Book upsert는 과제 생성보다 **먼저** 실행된다.

**mutation별 사전조건·실제 커밋 행·스냅샷 차이·실패 테스트.**

| mutation | 지정 테스트 | 테스트 fixture 사전 상태 | 트랜잭션 밖에서 **실제로 커밋되는 행** | 실패 시점 (hook) | 실패 후 스냅샷 차이 |
|---|---|---|---|---|---|
| **29b** | **18** | Book `Big Note` 1건 + 블록 1건 + 과제 1건 | Book: `Big Note` **update**(`update: {}`이나 `@updatedAt`이 갱신됨) + 나머지 **8권 create** | 3단계 `beforeCreateHook` | Book 1행 → **9행**, `Big Note`의 `updatedAt` 변경 |
| **29b** | **18c** | 같음 | 같음 (Book 9권) | 5단계 `afterBlocksHook` | 같음 |
| **29c** | **18b** | **빈 DB** | Book **9권 create** + 블록 **10건 create**(빈 DB이므로 `count()===0`) | 5단계 `afterBlocksHook` | 0/0/0 → Book **9**, 블록 **10**, 과제 0 |
| **29c** | **18c** | Book 1 + 블록 1 + 과제 1, `force: true` | Book 9권(1 update + 8 create) → `force` 삭제로 과제 1건·블록 1건 **삭제 커밋** → 블록 `count()===0`이 되어 **10건 create** | 5단계 `afterBlocksHook` | Book 1→**9**, 블록 `사용자 블록` 1건 → **시드 10건**, 과제 1→**0** |
| **29d** | **18d** | Book 1 + 블록 1 + 과제 1, `force: true` | 29c와 같은 커밋에 더해 과제 `count()===0`이 되어 **27건 create**(1단계 Book이 이미 커밋돼 `bookId` 외래키가 성립한다) | 7단계 `afterAssignmentsHook` | Book 1→**9**, 블록 1→**10**, 과제 `사용자 과제` 1건 → **시드 27건** |
| **29e** | **18·18b·18c·18d** | 각 테스트의 fixture | 해당 테스트의 hook이 던지기 **직전까지의 모든 단계** | 각 테스트가 주입한 hook | 위 네 행의 차이가 각각 그대로 남는다 |

**생성이 실제로 일어난다는 증명 — fixture의 기존 키와 생성 대상의 unique key 대조.**

| 대상 | 생성 가드 | fixture가 갖는 키 | 생성 대상의 키 | mutation 실행 시점의 가드 값 | 결론 |
|---|---|---|---|---|---|
| `Book` | **가드 없음.** 9권을 `title` 기준으로 항상 upsert한다 (`Book_title_key`, T03 요구사항 20) | `Big Note` 1건 — **시드 9권 중 하나다**(정상 케이스 19) | 시드 표의 9개 `title` | — | `Big Note`는 **update 1건**(`@updatedAt` 갱신), 나머지 **8건은 create**. 어떤 fixture에서도 skip이 없다 |
| `ScheduleBlock` | `count() === 0` | `date="2026-08-20"`인 사용자 블록 1건 (unique 제약 없음) | `date="2026-07-29"` 10건 | 18b: 빈 DB → **0**. 18c·18d: **2단계 `force` 삭제가 먼저 커밋되므로 0** | **10건 전부 create.** skip 불가 |
| `Assignment` | `count() === 0` | `date="2026-08-20", orderIndex=0` 사용자 과제 1건 (unique 제약 없음) | `2026-07-29`~`2026-08-17`의 27건, `bookId`는 1단계 Book 참조 | 18d: **2단계 `force` 삭제가 먼저 커밋되므로 0** | **27건 전부 create.** `bookId` 외래키는 1단계가 이미 커밋했으므로 성립 |

- **`force`가 없는 18b는 빈 DB이므로** 삭제 없이도 두 가드가 0이다. **`force`가 있는 18c·18d는** 삭제가 생성 가드보다 앞 단계이므로 가드가 0이 된다. 두 경로 모두 **생성이 no-op이 되는 조합이 없다.**
- 실패 원인은 언제나 **트랜잭션 밖에서 이미 커밋된 실제 행**이다. 존재하지 않는 ID, 별도 writer 연결, `SQLITE_BUSY`, 타이밍 경쟁을 쓰지 않는다.
- 판정은 **전체 DB 스냅샷의 deep equality**이며 실패 후 스냅샷은 **새 `PrismaClient`** 로 읽는다 (요구사항 2-B). 이 원칙은 바뀌지 않는다.
- **정상 구현에서는 네 테스트가 모두 통과한다.** 경계가 1단계부터 7단계 전체를 감싸면 어떤 hook에서 던져도 1~7단계의 모든 쓰기가 함께 롤백되어 스냅샷이 호출 전과 같아진다.

**mutation 교차 검토 — 같은 거짓 양성이 다른 곳에 없는지 (R2-04 Round 4 요구 7).**

| mutation | 조건부 생성 가드에 걸려 skip될 수 있는가 | 선행 단계(외래키·삭제)가 깨지는가 | 지정 테스트가 결정적으로 실패하는가 |
|---|---|---|---|
| 29b | **아니오** — Book upsert에는 가드가 없다 | 아니오 — 1단계가 원래 첫 단계다 | 예 (18·18c) |
| 29c | **아니오** — 삭제(2단계)가 가드(4단계)보다 앞에서 커밋된다 | 아니오 — 1~3단계가 순서대로 선행한다 | 예 (18b·18c) |
| 29d | **아니오** — 같은 이유로 두 가드가 모두 0이다 | 아니오 — Book upsert(1단계)가 과제 생성(6단계)보다 앞선다 | 예 (18d) |
| 29e | **아니오** — 순서가 원본과 동일하다 | 아니오 | 예 (18·18b·18c·18d) |

**mutation 명령은 전부 열 개다 — 27 · 28 · 28b · 29 · 29b · 29c · 29d · 29e(`prisma/seed.ts` 8개) + 31 · 32(`package.json` 2개, 아래 "`db:setup` mutation" 절).** 열 명령의 실제 출력을 완료 보고에 포함하고, 코드는 원상 복구된 상태여야 한다. 원복 후 `git diff prisma/seed.ts`가 비어 있어야 한다 (`package.json`은 요구사항 18의 변경만 남는다).
...
**왜 "두 번째 writer 연결"이 아니라 "트랜잭션 경계 미루기"인가 (R2-04 Round 3 잔여 2).** Round 2까지의 mutation은 트랜잭션이 **열려 있는 동안** 별도 클라이언트로 같은 SQLite 파일에 쓰게 했다. SQLite는 writer를 하나만 허용하므로 그 쓰기는 `SQLITE_BUSY`/timeout으로 거절될 수 있고, 그러면 **행이 하나도 남지 않은 채** 호출이 reject되어 잘못된 구현이 롤백 테스트를 통과한다 — mutation의 실패 이유가 "부분 저장"이 아니라 "잠금"이 되는 거짓 양성이다. 현재 네 mutation은 **경계 앞의 단계를 단일 연결로 순차 커밋한 뒤에야** 트랜잭션을 열므로 동시 writer도 `SQLITE_BUSY`도 발생하지 않는다.

| 성질 | Round 2 mutation (제거됨) | 현재 mutation 29b~29e |
|---|---|---|
| 두 번째 writer 연결 | 필요 | **없음** — 시종일관 `client` 하나 |
| `SQLITE_BUSY` 가능성 | 있음 | **없음** — 트랜잭션 열림 구간과 겹치지 않는다 |
| 실패 seam이 던질 때의 DB 상태 | 불확정 (쓰기가 거절됐을 수 있음) | **경계 앞 단계의 write가 전부 이미 커밋됨** — 생성 가드가 skip으로 빠지는 조합이 없다 (Round 4 교차 검토 표) |
...
완료 보고에 위 스크립트의 **실제 출력 전문**과, **mutation 케이스 열 개(27 · 28 · 28b · 29 · 29b · 29c · 29d · 29e · 31 · 32)** 의 출력을 포함한다.
